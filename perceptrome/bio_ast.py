...
SCHEMA_VERSION = 3
STABLE_NODE_KINDS = frozenset({"genome", "plasmid", "virus", "gene", "orf", "cds", "region", "domain", "sme", "residue", "kmer", "microfeature"})
CONTAINMENT_EDGE_KIND = "contains"
SEMANTIC_EDGE_KINDS = frozenset({"regulates", "overlaps", "adjacent_to", "homologous_to"})
...
class HierarchicalNode:
    ...
    def _infer_canonical_id(self) -> str:
        return ""
    def to_dict(self) -> Dict[str, Any]:
        return {"node_type": self.node_type, "canonical_id": self.canonical_id, "parent_id": self.parent_id, "child_ids": list(self.child_ids), "start": self.start, "end": self.end, "strand": self.strand, "frame": self.frame, "metadata": dict(self.metadata)}
    @classmethod
    def _base_kwargs(cls: Type[TNode], payload: Mapping[str, Any]) -> Dict[str, Any]:
        child_ids_raw = payload.get("child_ids")
        return {"canonical_id": str(payload.get("canonical_id", "")), "parent_id": str(payload["parent_id"]) if payload.get("parent_id") is not None else None, "child_ids": tuple(str(v) for v in child_ids_raw) if isinstance(child_ids_raw, list) else (), "start": int(payload["start"]) if isinstance(payload.get("start"), int) else None, "end": int(payload["end"]) if isinstance(payload.get("end"), int) else None, "strand": str(payload["strand"]) if payload.get("strand") is not None else None, "frame": int(payload["frame"]) if isinstance(payload.get("frame"), int) else None, "metadata": dict(payload.get("metadata", {})) if isinstance(payload.get("metadata"), Mapping) else {}}


@dataclass(frozen=True)
class GenomeNode(HierarchicalNode): node_type: ClassVar[str] = "genome"
@dataclass(frozen=True)
class PlasmidNode(HierarchicalNode): node_type: ClassVar[str] = "plasmid"
@dataclass(frozen=True)
class VirusNode(HierarchicalNode): node_type: ClassVar[str] = "virus"
...
class GeneNode(HierarchicalNode):
    ...
    def _infer_canonical_id(self) -> str: return self.gene_id
    def __post_init__(self) -> None:
        if not self.gene_id and self.canonical_id:
            object.__setattr__(self, "gene_id", self.canonical_id)
        super().__post_init__()
    def to_dict(self) -> Dict[str, Any]:
        payload = super().to_dict(); payload.update({"gene_id": self.gene_id, "dtype": self.dtype, "value": self.value, "attributes": [a.to_dict() for a in self.attributes]}); return payload
    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "GeneNode":
        attrs = payload.get("attributes")
        return cls(**cls._base_kwargs(payload), gene_id=str(payload.get("gene_id", "")), dtype=payload.get("dtype"), value=payload.get("value"), attributes=tuple(AttributeNode.from_dict(item) for item in attrs if isinstance(item, Mapping)) if isinstance(attrs, list) else ())
...
@dataclass(frozen=True)
class ORFNode(HierarchicalNode): node_type: ClassVar[str] = "orf"
@dataclass(frozen=True)
class CDSNode(HierarchicalNode): node_type: ClassVar[str] = "cds"
@dataclass(frozen=True)
class DomainNode(HierarchicalNode): node_type: ClassVar[str] = "domain"
@dataclass(frozen=True)
class RegionNode(HierarchicalNode): node_type: ClassVar[str] = "region"
...
class SMENode(HierarchicalNode):
    ...
    def __post_init__(self) -> None:
        ...
        super().__post_init__()
    def to_dict(self) -> Dict[str, Any]:
        payload = super().to_dict(); payload.update({"secondary_tag": self.secondary_tag, "motif_family": self.motif_family, "motif_subtype": self.motif_subtype, "energetic_evolutionary": self.energetic_evolutionary.to_dict()}); return payload
    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "SMENode":
        return cls(**cls._base_kwargs(payload), secondary_tag=payload.get("secondary_tag"), motif_family=payload.get("motif_family"), motif_subtype=payload.get("motif_subtype"), energetic_evolutionary=EnergeticEvolutionaryPayload.from_dict(payload.get("energetic_evolutionary")))

@dataclass(frozen=True)
class ResidueNode(HierarchicalNode): node_type: ClassVar[str] = "residue"
@dataclass(frozen=True)
class KmerNode(HierarchicalNode): node_type: ClassVar[str] = "kmer"
@dataclass(frozen=True)
class MicrofeatureNode(HierarchicalNode): node_type: ClassVar[str] = "microfeature"

ASTNode = Union[GenomeNode, PlasmidNode, VirusNode, GeneNode, ORFNode, CDSNode, DomainNode, RegionNode, SMENode, ResidueNode, KmerNode, MicrofeatureNode]
_NODE_CLASS_BY_TYPE: Dict[str, Type[HierarchicalNode]] = {cls.node_type: cls for cls in (GenomeNode, PlasmidNode, VirusNode, GeneNode, ORFNode, CDSNode, DomainNode, RegionNode, SMENode, ResidueNode, KmerNode, MicrofeatureNode)}
...
def _normalize_window_positions(window: Optional[Iterable[Union[int, Tuple[int, int]]]]) -> Tuple[Tuple[int, int], ...]:
    ...
    for item in window:
        if isinstance(item, int): start, end = item, item
        elif isinstance(item, tuple) and len(item) == 2 and all(isinstance(v, int) for v in item): start, end = item
        else: raise ValueError("Window values must be ints or (start, end) integer tuples")
        if start > end: raise ValueError("Window start must be <= end")
        normalized.append((start, end))
    return tuple(normalized)


def build_sme_node(*, parent: Union[DomainNode, RegionNode], sme_id: str, residue_window: Optional[Iterable[Union[int, Tuple[int, int]]]] = None, kmer_window: Optional[Iterable[Union[int, Tuple[int, int]]]] = None, secondary_tag: Optional[str] = None, motif_family: Optional[str] = None, motif_subtype: Optional[str] = None, energetic_evolutionary: Optional[Mapping[str, Any]] = None, metadata: Optional[Mapping[str, Any]] = None) -> Tuple[Union[DomainNode, RegionNode], SMENode, Tuple[Union[ResidueNode, KmerNode], ...]]:
    ...
    for index, (start, end) in enumerate(residue_ranges, start=1):
        cid = f"residue:{sme_id}:{index}"; child_ids.append(cid); child_nodes.append(ResidueNode(canonical_id=cid, parent_id=sme_id, start=start, end=end))
    for index, (start, end) in enumerate(kmer_ranges, start=1):
        cid = f"kmer:{sme_id}:{index}"; child_ids.append(cid); child_nodes.append(KmerNode(canonical_id=cid, parent_id=sme_id, start=start, end=end))
    sme_node = SMENode(canonical_id=sme_id, parent_id=parent.canonical_id, child_ids=tuple(child_ids), secondary_tag=secondary_tag, motif_family=motif_family, motif_subtype=motif_subtype, energetic_evolutionary=EnergeticEvolutionaryPayload.from_dict(energetic_evolutionary), metadata=dict(metadata) if isinstance(metadata, Mapping) else {})
...
class RelationshipEdge:
    ...
    @property
    def relation(self) -> str: return self.kind
    @property
    def source_gene_id(self) -> str: return self.source_id
    @property
    def target_gene_id(self) -> str: return self.target_id
    def to_dict(self) -> Dict[str, Any]:
        return {"source_id": self.source_id, "target_id": self.target_id, "kind": self.kind, "relation": self.kind, "metadata": dict(self.metadata), "source_gene_id": self.source_id, "target_gene_id": self.target_id}
    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "RelationshipEdge":
        return cls(source_id=str(payload.get("source_id", payload.get("source_gene_id", ""))), target_id=str(payload.get("target_id", payload.get("target_gene_id", ""))), kind=str(payload.get("kind", payload.get("relation", ""))), metadata=dict(payload.get("metadata", {})) if isinstance(payload.get("metadata"), Mapping) else {})
...
class BioAST:
    ...
    @property
    def hierarchical_nodes(self) -> Tuple[ASTNode, ...]: return self.nodes
    @property
    def semantic_edges(self) -> Tuple[RelationshipEdge, ...]: return self.relationships
    @property
    def containment_edges(self) -> Tuple[RelationshipEdge, ...]: return tuple(edge for edge in self.edges if edge.kind == CONTAINMENT_EDGE_KIND)
    @property
    def node_by_id(self) -> Dict[str, ASTNode]: return {node.canonical_id: node for node in self.nodes}
    def to_dict(self) -> Dict[str, Any]:
        return {"schema_version": self.schema_version, "sequence_metadata": self.sequence_metadata.to_dict(), "nodes": [n.to_dict() for n in self.nodes], "hierarchical_nodes": [n.to_dict() for n in self.nodes], "edges": [e.to_dict() for e in self.edges], "relationships": [e.to_dict() for e in self.relationships], "genes": [g.to_dict() for g in self.genes]}
    @classmethod
    def from_dict(cls, payload: Optional[Mapping[str, Any]]) -> "BioAST":
        if not isinstance(payload, Mapping): return cls()
        ...
        return cls(genes=genes, nodes=nodes or tuple(genes), edges=edges, sequence_metadata=SequenceMetadata.from_dict(payload.get("sequence_metadata")), schema_version=SCHEMA_VERSION)
...
def migrate_bio_ast_payload(payload: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {"schema_version": SCHEMA_VERSION, "sequence_metadata": seq_meta.to_dict(), "nodes": [node.to_dict() for node in node_objs], "edges": [edge.to_dict() for edge in (_compute_containment_edges(node_objs) + tuple(RelationshipEdge.from_dict(edge) for edge in semantic_edges))], "relationships": semantic_edges, "genes": [node.to_dict() for node in node_objs if isinstance(node, GeneNode)]}


def ast_from_flat_genes_payload(payload: Optional[Mapping[str, Any]]) -> BioAST:
    if not isinstance(payload, Mapping): return BioAST()
    if "schema_version" in payload or "nodes" in payload: return BioAST.from_dict(payload)
    raw_genes = payload.get("genes") if "genes" in payload else payload
    if not isinstance(raw_genes, Mapping): return BioAST()
...
def ast_to_flat_genes_payload(ast: BioAST) -> Dict[str, Any]: return {"genes": {gene.gene_id: gene.value for gene in ast.genes}}
...
def registry_ast_from_definitions(definitions: Iterable[Mapping[str, Any]]) -> BioAST:
    ...
    for definition in definitions:
        ...
        nodes.append(GeneNode(gene_id=gene_id, canonical_id=gene_id, dtype=str(definition["dtype"]), value=definition.get("default"), metadata={"mutation_rate": definition["mutation_rate"], "novelty_weight": definition["novelty_weight"], "min_value": definition.get("min_value"), "max_value": definition.get("max_value"), "choices": definition.get("choices")}))
...
def definition_map_from_registry_ast(ast: BioAST) -> Dict[str, Dict[str, Any]]:
    ...
    return {node.gene_id: {"id": node.gene_id, "dtype": node.dtype, "mutation_rate": float(node.metadata.get("mutation_rate", 0.0)), "novelty_weight": float(node.metadata.get("novelty_weight", 0.0)), "default": node.value, "min_value": node.metadata.get("min_value"), "max_value": node.metadata.get("max_value"), "choices": node.metadata.get("choices"), "depends_on": deps.get(node.gene_id)} for node in ast.genes}
