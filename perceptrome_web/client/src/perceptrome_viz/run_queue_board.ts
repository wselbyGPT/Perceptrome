...
export class RunQueueBoard {
  private statusEl = mustEl<HTMLElement>("status");
  private summaryEl = mustEl<HTMLElement>("run-summary-cards");
  private activeEl = mustEl<HTMLElement>("active-run-board");
  private failedEl = mustEl<HTMLElement>("failed-run-board");
  ...
  renderSummary(summary: RunSummary): void {
    ...
    this.summaryEl.innerHTML = cards
      .map(([label, count]) => `<article class="mini-card"><h3>${label}</h3><p>${count}</p></article>`)
      .join("");
  }
